...
public class Adult extends Customer implements CustomerDao,ItemDao {
    private HashMap<String,Integer>  pointEarned= new HashMap<String,Integer>();
	private int totalPointsEarned=0;
    public Adult() throws Exception {
		super();
		prepareEarnedPointForCustomer();
	}
		public Adult(String phone) {
		super(phone);}
...
public void prepareEarnedPointForCustomer() throws Exception {
 ArrayList<Item> items=getAllItem();
	    ...
	    if(item.getCode()==1) {
			...
			this.pointEarned.put(item.getName(),10);
		}else if(item.getCode()==4) {
			this.pointEarned.put(item.getName(),25);
		}
 }
}
@Override
public void editCustomer(Customer cus) throws Exception {
	ArrayList<Customer> customers=getAllCustomerByType("adult");
